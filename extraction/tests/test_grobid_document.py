...
from aicacia_extraction.grobid import IDNO, IDNOType, GROBIDDocument
from pathlib import Path


class TestGROBIDDocument(unittest.TestCase):
    def test_grobid_document(self):
        # Given
        tests_path = Path(os.path.dirname(__file__))
        tei_path = tests_path / "../example/grobid/sample.grobid.tei.xml"

        # When
        document = GROBIDDocument.from_path(tei_path)
        ...
        self.assertEqual(["Lionel Eyraud", "Gr Égory Mouni", "Denis Trystram"], document.authors)
        self.assertEqual(
            [
        ...
        self.assertEqual([IDNO(IDNOType.MD5, "5F0BE4D19AC38AC86BE01245CF0BEAF4")], document.idnos)
        self.assertEqual("Bi-criteria Algorithm for Scheduling Jobs on Cluster Platforms", document.title)

    def test_grobid_document_authors(self):
        # Given
        f = io.StringIO("""
        <sourceDesc>
            <biblStruct>
        ...
        # When
        document = GROBIDDocument(f)

        # Then
        self.assertEqual(["John", "Doe", "John Doe"], document.authors)

        # Cleanup
        f.close()

    def test_grobid_document_idnos(self):
        # Given
        f = io.StringIO("""
        <sourceDesc>
        ...
        self.assertEqual("10.1371/journal.pone.0170706", document.doi)
