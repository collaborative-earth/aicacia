...
class CorpusGenerator:
    ...
    def load_corpus(self, corpus_path: Optional[str] = None) -> List[TextNode]:
        """
        Load corpus from a JSONL file and reconstruct TextNode objects.

        Args:
            corpus_path: Optional path to the corpus JSONL file. 
                         If None, uses the default path generated by _generate_corpus_path().

        Returns:
            List of TextNode objects reconstructed from the corpus file.
        """
        path = corpus_path or self._generate_corpus_path()
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                node = TextNode(
                    text=data.get("text", ""),
                    id_=data.get("_id", str(uuid.uuid4())),
                    metadata={"title": data.get("title", "")}
                )
                nodes.append(node)

        print(f"Loaded {len(nodes)} nodes from corpus: {path}")
        return nodes
    ...
    def generate_corpus(
        ...
        if nodes is not None:
            ...
            final_nodes = self.process_documents(documents)
        if len(final_nodes) > self.config.max_nodes:
            final_nodes = final_nodes[:self.config.max_nodes]
            print(f"Limiting to max_nodes: {self.config.max_nodes}")
